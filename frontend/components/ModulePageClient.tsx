...
interface ModulePageClientProps {
  moduleId: string;
  initialModule: ModuleExtended | null;
  initialModuleError: string | null;
}
...
export default function ModulePageClient({
  ...
  const [module, setModule] = useState<ModuleExtended | null>(initialModule);
  const [moduleError, setModuleError] = useState<string | null>(initialModuleError);
  ...
  useEffect(() => {
    ...
    if (!initialModule) {
      fetchModule(moduleId)
        ...
        .catch((error) => {
          console.error(error);
          setModuleError(initialModuleError || "Module could not be loaded from the backend.");
        });
    }
    ...
    fetchProgress(userKey)
      .then(setProgress)
      .catch(console.error);
  }, [moduleId, initialModule, initialModuleError]);

  if (!module) {
    return (
      <div className="flex items-center justify-center min-h-screen px-6 text-center">
        <div className="max-w-sm rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6">
          <p className="text-sm font-semibold text-[var(--color-text-primary)]">Module unavailable</p>
          <p className="mt-2 text-sm text-[var(--color-text-secondary)]">
            {moduleError || "Loading module..."}
          </p>
          <Link
            href="/curriculum"
  ...
  const totalCount = module.lesson_count ?? module.lessons?.length ?? 0;
      ...
      {viewMode === "list" && (
        <div className="space-y-2">
          {(module.lessons ?? []).map((lesson) => {
            ...
            const isCapstone = lesson.is_capstone;
